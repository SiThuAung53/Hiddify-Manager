...
function get_commit_version() {
    json_data=$(curl -sL -H "Accept: application/json" "https://github.com/SiThuAung53/$1/commits/main.atom")
    latest_commit_date=$(echo "$json_data" | jq -r '.payload.commitGroups[0].commits[0].committedDate')
    # xml_data=$(curl -sl "https://github.com/SiThuAung53/$1/commits/main.atom")
    # latest_commit_date=$(echo "$xml_data" | grep -m 1 '<updated>' | awk -F'>|<' '{print $3}')
    # COMMIT_URL=$(curl -s https://api.github.com/repos/SiThuAung53/$1/git/refs/heads/main | jq -r .object.url)
...
function get_pre_release_version() {
    ...
    VERSION=$(curl -sL "https://api.github.com/repos/SiThuAung53/$1/releases" | jq -r 'map(select(.prerelease == true or .draft == true)) | sort_by(.created_at) | last | .tag_name')
    VERSION=${VERSION/#v/}
    echo $VERSION
}

function get_release_version() {
    VERSION=$(curl -sL "https://api.github.com/repos/SiThuAung53/$1/releases" | jq -r 'map(select(.prerelease == false)) | sort_by(.created_at) | last | .tag_name')
    if [ -z $VERSION ]; then
        # COMMIT_URL=https://api.github.com/repos/SiThuAung53/$1/releases/latest
        # VERSION=$(curl -s --connect-timeout 1 $COMMIT_URL | jq -r .tag_name)
        location=$(curl -sI "https://github.com/SiThuAung53/$1/releases/latest" | grep -i location | awk -F' ' '{print $2}' | tr -d '\r')
